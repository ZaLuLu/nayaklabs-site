...
export function EventMeshRadar() {
  ...
  return (
    <div className="w-full bg-[var(--bg-card)] border border-[var(--border-base)] p-5 sm:p-6 text-left flex flex-col gap-4 rounded-xl shadow-lg font-mono">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-[var(--border-base)]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-[var(--text-primary)] tracking-wider uppercase">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 bg-[var(--bg-surface)] border border-[var(--border-base)] text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--accent-primary)] rounded-md w-44 sm:w-56 transition-colors"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 min-h-[170px]">
        {filteredEvents.length === 0 ? (
          <div className="col-span-2 flex items-center justify-center p-8 text-xs text-[var(--text-muted)]">
            No active events matching your filter criteria.
          </div>
        ) : (
          filteredEvents.map((evt) => {
            ...
            return (
              <div
                key={evt.id}
                className="bg-[var(--bg-surface)]/80 border border-[var(--border-base)] hover:border-[var(--border-hover)] p-4 flex flex-col justify-between transition-all rounded-lg group relative"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="px-2 py-0.5 bg-[var(--bg-card)] border border-[var(--border-base)] text-[var(--text-secondary)] text-[11px] font-semibold uppercase tracking-wider rounded-md">
                      {evt.category}
                    </span>
                    {evt.isHot && (
                ...
                <div className="flex items-center justify-between pt-3 mt-3 border-t border-[var(--border-base)] text-xs">
                  <span className="text-[var(--text-muted)]">{evt.attendees} builders</span>

                  <button
                    onClick={(e) => handleRsvp(evt.id, e)}
                    className={`px-3 py-1 flex items-center gap-1.5 text-xs font-semibold transition-colors cursor-pointer rounded-md ${
                      isJoined
